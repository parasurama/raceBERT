...
tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased",
                                          use_fast=True)
nlp = TextClassificationPipeline(model=model, tokenizer=tokenizer, device=0)
...
precision_recall_fscore_support(df['race_cat'].values, df['pred_race'].values, average="weighted")

precision, recall, f1, support = precision_recall_fscore_support(df['race_cat'].values, df['pred_race'].values, average=None, labels=[0, 1, 2, 3])
